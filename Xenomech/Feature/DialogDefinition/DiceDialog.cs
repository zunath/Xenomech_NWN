    ...
    public class DiceDialog: DialogBase
    {
        ...
        private void MainPageInit(DialogPage page)
        {
            ...
            if (!GetIsDM(player))
            {
                ...
                page.AddResponse("Groups", () =>
                {
                    ChangePage(GroupPageId);
                });
            }
        ...
        private void GroupPageInit(DialogPage page)
        {
            ...
            page.AddResponse("Ether", () =>
            {
                model.Group = DiceGroup.Ether;
                model.Skill = SkillType.Invalid;
            });

            page.AddResponse("Healing", () =>
            {
                model.Group = DiceGroup.Healing;
                model.Skill = SkillType.Invalid;
            });
        }
            ...
            if (!GetIsDM(user))
            {
                ...
                if (model.Group != DiceGroup.None)
                {
                    switch (model.Group)
                    {
                        ...
                        case DiceGroup.Healing:
                            totalSkill = dbPlayer.Skills[SkillType.Arcane].Rank +
                                         dbPlayer.Skills[SkillType.FirstAid].Rank;
                            modifier = totalSkill / 2;
                            message = ColorToken.SkillCheck("Dice Roll [HEALING]: ") + dieRoll + "+" + modifier + ": " + (value + modifier);

                            break;
                    }
                }
                // Skills
                else if (model.Skill != SkillType.Invalid)
                {
                    var skillDetail = Skill.GetSkillDetails(model.Skill);
                    totalSkill = dbPlayer.Skills[model.Skill].Rank;
                    modifier = totalSkill / 2;
